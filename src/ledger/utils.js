...
import {
  APP_KEY, CHUNK_SIZE, CLA, INS, errorCodeToString, getVersion, processErrorResponse,
} from './common';
...
export default class DecimalApp {
  ...
  async getAddressAndPubKey(path, hrp) {
    ...
    const response = await this.transport.send(CLA, INS.GET_ADDR_SECP256K1, 1, 0, data, [0x9000]);
    const errorCodeData = response.slice(-2);
    const returnCode = errorCodeData[0] * 256 + errorCodeData[1];
